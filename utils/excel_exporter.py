...
def generate_students_query_excel(students_data, template_path):
    ...
    # [内部编号, 姓名*, 省学籍辅号, 国家身份证号, 性别, 校区*, 入学年份*, 当前班级*, 选科*, 类别*, 专业, 在校情况*, 住宿情况*, 曾用名, 公寓, 宿舍及床位, 特殊情况备注]
    for row_idx, student in enumerate(students_data, start=2):  # 从第2行开始写
        ...
        sheet.cell(row=row_idx, column=10, value=student.get('category', ''))
        sheet.cell(row=row_idx, column=11, value=student.get('major', ''))
        sheet.cell(row=row_idx, column=12, value=student.get('at_school', ''))
        sheet.cell(row=row_idx, column=13, value=student.get('boarding_status', ''))
        sheet.cell(row=row_idx, column=14, value=student.get('former_name', ''))
        sheet.cell(row=row_idx, column=15, value=student.get('apartment', ''))
        sheet.cell(row=row_idx, column=16, value=student.get('dormitory', ''))
        sheet.cell(row=row_idx, column=17, value=student.get('remarks', ''))
